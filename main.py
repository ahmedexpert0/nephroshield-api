# main.py - CKD Prediction API with XAI and Gemini
...
# ---- Gemini import (google-genai package) ----
try:
...
def get_reference_value(feature):
    reference_values = {
        "LBXHGB": 13.8, "LBXTC": 190, "LBXTR": 110,
        "BPXSY1": 120, "BPXDI1": 80, "BMXBMI": 27,
        "LBXGLU": 95, "LBXGH": 5.4, "RIDAGEYR": 50,
        "URXUCR": 100, "URXUMA": 10, "LBXSUA": 5.0,
    }
...
def call_nvidia_xai_api(patient_data, features_list):
    ...
    try:
        ...
        payload = {
            ...
            "output_features": features_list[:20]
        }
...
def generate_abnormal_labs_summary(patient_data):
    ...
    for key, (low, high, unit) in reference_ranges.items():
        if key in patient_data:
            ...
            if val < low:
                abnormal.append(f"- {FEATURE_DESCRIPTIONS.get(key, key)}: {val} {unit} (below normal {low}-{high})")
            elif val > high:
                abnormal.append(f"- {FEATURE_DESCRIPTIONS.get(key, key)}: {val} {unit} (above normal {low}-{high})")
...
async def predict_ckd(payload: PredictionRequest):
    try:
        ...
        return {
            ...
            "nvidia_explanations": nvidia_explanations,
            "personalized_rapport": personalized_rapport,
